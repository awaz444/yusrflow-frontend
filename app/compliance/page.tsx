...
import {
  ComplianceIssues,
  ComplianceIssue,
} from "@/components/compliance/compliance-issues";
...
export default function CompliancePage() {
  ...
  const handleDownloadReport = async (lang: string = "en") => {
    ...
    try {
      ...
      await downloadFile(
        `/reports/compliance-health/pdf?lang=${lang}`,
        `Yusrflow_Compliance_Report_${lang.toUpperCase()}_${timestamp}.pdf`,
      );
    } catch (error) {
  ...
  if (loading) {
    return (
      <PageContainer>
        <LoadingState
          message={t("compliance.loading") || "Loading compliance data..."}
        />
      </PageContainer>
    );
  }
      ...
      {isScanning && (
        <Card className="mb-8 p-6 border-purple-500/30 bg-gradient-to-r from-purple-500/10 to-blue-500/5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-foreground">
                  AI Compliance Scan Running
                </p>
                <p className="text-sm text-muted-foreground">
                  Currently evaluating:{" "}
                  <span className="text-purple-300">
                    {scanProgress.lastApp}
                  </span>
                </p>
              </div>
            </div>
          ...
          <p className="text-xs text-muted-foreground mt-2">
            {scanProgress.total > 0
              ? `${Math.round((scanProgress.scanned / scanProgress.total) * 100)}% complete`
              : "Initializing..."}
          </p>
        </Card>
      )}
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="p-6 bg-gradient-to-br from-blue-500/10 to-blue-500/5 border-blue-500/30">
          <h3 className="font-semibold text-blue-400 mb-2">
            {t("compliance.upcomingDeadlines")}
          </h3>
          <ul className="text-sm text-muted-foreground space-y-2">
        ...
        <DialogContent className="sm:max-w-md border-purple-500/30 bg-background/95 backdrop-blur-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground text-base leading-relaxed">
              ...
              plan allows scanning{" "}
              <span className="text-foreground font-bold">up to 12 apps</span>{" "}
              per session.
              <br />
              <br />
              Clicking{" "}
              <span className="text-foreground font-semibold">Okay</span> will
              proceed with scanning the first 12 applications in your inventory.
            </DialogDescription>
          </DialogHeader>
